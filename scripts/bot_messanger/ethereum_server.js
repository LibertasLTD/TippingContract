  ...
  ipc.serve(() => {
    ...
    ipc.server.on("message", (data, socket) => {
      ...
      bridge
        ...
        .then((receipt) => {
          ipc.log(
            `bridging to end performed with args: ${params} at ${receipt.transactionHash}`,
            data
          );
        })
        .catch((error) => {
      ...
      instance
        ...
        .on("data", (event) => {
          ...
          ipc.server.broadcast("message", {
            from: ipc.config.id,
            message: `${eventData._tokenAtStart}|${eventData._from}|${eventData._to}|${eventData._amount}|LIBERTAS|LIBS`,
          });
        })
