          ...
          IERC20.at(endToken).then((token) => {
            ...
            token.approve(instance.address, amountToBridge, { from: accounts[0] }).then(() => {
              token.balanceOf(accounts[0]).then((balance) => {
                ...
                instance.requestBridgingToStart(startToken, token.address, accounts[0], amountToBridge, { from: accounts[0] })
                  .then((receipt) => {
                    console.log(receipt);
                    token.balanceOf(accounts[0]).then((newBalance) => {
                      console.log(`Performed requesting bridge to start with new balance of ${newBalance}`);
                      callback();
                    });
                  }).catch((err) => {
                    console.error(err);
