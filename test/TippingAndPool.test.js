...
contract('Tipping', accounts => {
    ...
    beforeEach(async function() {
        ...
        bridgedStandartERC20 = await BridgedStandardERC20.new({ from: account_one });
        bridge2 = await Bridge.new(false, bridgedStandartERC20.address, account_three, libertas2.address, "LIBERTAS", "ODEUM", { from: account_one });
        libertas = await BridgedStandardERC20.at(await bridge2.getEndTokenByStartToken(libertas2.address));

        await bridge2.performBridgingToEnd(libertas2.address, account_one, 1000, "LIBERTAS", "ODEUM", {from: account_three});
        ...
        tipping = await Tipping.new(stakingPool.address, libertas.address, vault, burn, burnRate, fundRate, rewardRate);
        // 0x00 - admin
