...
contract("Bridge", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    mockToken = await MockToken.new("Mock Token", "MT", ether("100"), {
      from: owner,
    });
  });
  ...
  describe("non ERC20 standard functionality in BridgedStandardERC20", () => {
    it("should get name and symbol", async () => {
      ...
      const tokenAtEnd = await IBridgedStandardERC20.at(
        await bridge2.getEndTokenByStartToken(libertas.address)
      );
    ...
    it("should transfer safely if recipient is not zero address", async () => {
      ...
      expect(await tokenAtEnd.balanceOf(bob)).to.be.bignumber.equal(
        tokensToBridge
      );
    });
  });
  ...
  it("should evacuate tokens", async () => {
    await expectRevert(
      bridge1.evacuateTokens(libertas.address, ZERO, ZERO_ADDRESS, {
        from: alice,
      }),
      "onlyAdmin"
    );
    ...
    await bridge1.evacuateTokens(mockToken.address, amountToEvac, alice, {
      from: owner,
    });
    expect(await mockToken.balanceOf(alice)).to.be.bignumber.equal(
      amountToEvac
    );
  });
  ...
  it("should set allowed token", async () => {
    await expectRevert(
      bridge1.setAllowedToken(mockToken.address, true, { from: alice }),
      "onlyAdmin"
    );
  ...
  it("should request bridging to end", async () => {
    ...
    await expectRevert(
      bridge1.requestBridgingToEnd(ZERO_ADDRESS, owner, tokensToBridge, {
        from: alice,
      }),
      "invalidToken"
    );
    const receipt = await bridge1.requestBridgingToEnd(
      libertas.address,
      alice,
      tokensToBridge
    );
    ...
    expect(await libertas.balanceOf(bridge1.address)).to.be.bignumber.equal(
      tokensToBridge
    );
  });
  ...
  it("should request bridging to start", async () => {
    ...
    await expectRevert(
      bridge2.requestBridgingToStart(
        libertas.address,
        ZERO_ADDRESS,
        owner,
        tokensToBridge,
    ...
    const receipt = await bridge2.requestBridgingToStart(
      libertas.address,
      tokenAtEnd.address,
      owner,
      tokensToBridge,
  ...
  it("should keep directions safe", async () => {
    await expectRevert(
      bridge2.requestBridgingToEnd(mock.address, ZERO_ADDRESS, ZERO),
      "onlyAtStart"
    );
    await expectRevert(
      bridge1.requestBridgingToStart(
        mock.address,
        mock.address,
        ZERO_ADDRESS,
        ZERO
    ...
    await expectRevert(
      bridge1.performBridgingToEnd(mock.address, ZERO_ADDRESS, ZERO, "", ""),
      "onlyAtEnd"
    );
    await expectRevert(
      bridge2.performBridgingToStart(
        ZERO_ADDRESS,
        mock.address,
        ZERO_ADDRESS,
        ZERO
  ...
  it("should create token at end if non exists", async () => {
    ...
    await bridge1.setAllowedToken(mockToken.address, true, { from: owner });
    await bridge1.requestBridgingToEnd(
      mockToken.address,
      alice,
      tokensToBridge,
      { from: owner }
    ...
    const tokenAtEndAddress = await bridge2.getEndTokenByStartToken(
      mockToken.address
    );
  ...
  it("should perform bridging from start to end and backwards", async () => {
    ...
    const receipt1 = await bridge2.performBridgingToEnd(
      libertas.address,
      alice,
      tokensToBridge,
      "",
    ...
    const tokenAtStart = await IBridgedStandardERC20.at(
      await bridge2.getStartTokenByEndToken(tokenAtEnd.address)
    );

    expect(tokenAtStart.address).to.be.equal(libertas.address);

    await tokenAtEnd.approve(bridge2.address, tokensToBridge, { from: alice });
    await bridge2.requestBridgingToStart(
      libertas.address,
      tokenAtEnd.address,
      owner,
      tokensToBridge,
    ...
    const receipt2 = await bridge1.performBridgingToStart(
      tokenAtStart.address,
      tokenAtEnd.address,
      owner,
      tokensToBridge,
      { from: owner }
  ...
  it("should revert if no start token provided", async () => {
    await expectRevert(
      bridge2.getStartTokenByEndToken(ZERO_ADDRESS),
      "noStartTokenFound"
    );
  });
  ...
  it("should revert if perform bridging called not by message bot", async () => {
    await expectRevert(
      bridge1.performBridgingToStart(
        ZERO_ADDRESS,
        ZERO_ADDRESS,
        ZERO_ADDRESS,
        ZERO,
    ...
    await expectRevert(
      bridge2.performBridgingToEnd(ZERO_ADDRESS, ZERO_ADDRESS, ZERO, "", "", {
        from: alice,
      }),
      "onlyMessangerBot"
    );
